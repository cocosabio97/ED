...
void  procesaArbol(const BinTree<string>& arbol, int& result, const MapTree<string, int>& dic) {
    ...
    if (arbol.empty()) {
        ...
        if (arbol.root() != "+" && arbol.root() != "-" && arbol.root() != "*") {
            ...
            //si no es una variable que este ya en el dicionario => que es un numero
            else {
...
bool tratar_caso() {
    ...
    if (numInstruciones == 0) {
        return false;
    }
    else {
        BinTree<std::string> instruccion;
        for (int i = 0; i < numInstruciones; i++) {
            MapTree<string, int>::MapEntry map(" ", 0);
