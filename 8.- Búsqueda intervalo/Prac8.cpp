...
private:
	...
	static void aux_interval(Node* root, const T& lower, const T& upper, int& result);
};
...
int SetTree<T>::elems_in_interval(const T& lower, const T& upper) const {
	// Implementar aquí. Puedes utilizar métodos auxiliares, si
	// los necesitas. En ese caso, decláralos arriba en la parte
	// privada de la clase, pero impleméntalos después de este método.
	int res = 0;
	aux_interval(this->root_node, lower, upper, res);
	return res;
}


// O (log n) siendo n el numero de elementos
template <typename T>
void SetTree<T>::aux_interval(Node* root, const T& lower, const T& upper, int& result) {

	// caso base
	if (root == nullptr)
		return;

	// si el elemento es mayor que la cota inferior puede haber elementos dentro rango menores que el
	if (root->elem > lower) {
		aux_interval(root->left, lower, upper, result);

	}
	// si el elemento es menor que la cota superior puede haber elementos dentro rango mayores que el
	if (root->elem < upper) {
		aux_interval(root->right, lower, upper, result);
	}

	// compruebo si el elemento esta en el rango
	if (root->elem >= lower && root->elem <= upper){
		result++;
	}
